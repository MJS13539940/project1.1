...
class Window_User:
    ...
    def create_widgets(self):    
        ...
        btn_user_list = Button(info_box, text='사용자 등록', padx=5, pady=5, 
                           command=None)
        btn_user_list.grid(row=0, column=0, columnspan=2, padx=5, pady=5, sticky='ew')
        # [ 새로 고침 ] 버튼
        btn_clear = Button(info_box, text='새로고침', padx=5, pady=5, width=10, 
                           command=self.clear)
        btn_clear.grid(row=8, column=0, padx=5, pady=5, sticky='ew')
        # [ 사용자 명단 ] 버튼
        btn_search = Button(info_box, text='사용자 명단', padx=5, pady=5, width=10, 
                            command=UserList)
        btn_search.grid(row=8, column=1, padx=5, pady=5, sticky='ew')

        #레이블
        label_user_num = Label(info_box, text="회원번호", anchor='e', justify='right')
        label_user_num.grid(row=2, column=0, padx=5, pady=5, sticky='ew')
        value_user_num = Entry(info_box)
        value_user_num.grid(row=2, column=1, padx=5, pady=5, sticky='ew')
        ...
        value_user_name.grid(row=3, column=1, padx=5, pady=5, sticky='ew')

        label_user_birth = Label(info_box, text="생년월일", anchor='e', justify='right')
        label_user_birth.grid(row=4, column=0, padx=5, pady=5, sticky='ew')
        value_user_birth = Entry(info_box)
        value_user_birth.grid(row=4, column=1, padx=5, pady=5, sticky='ew')

        label_user_phone = Label(info_box, text="연락처", anchor='e', justify='right')
        label_user_phone.grid(row=5, column=0, padx=5, pady=5, sticky='ew')
        value_user_phone = Entry(info_box)
        value_user_phone.grid(row=5, column=1, padx=5, pady=5, sticky='ew')

        label_user_etc = Label(info_box, text="비고", anchor='e', justify='right')
        label_user_etc.grid(row=6, column=0, padx=5, pady=5, sticky='ew')
        value_user_etc = Entry(info_box)
        value_user_etc.grid(row=6, column=1, padx=5, pady=5, sticky='ew')
...
###### 두번째 버튼을 누르면 열리는 페이지
class Window_Borrow:
    def __init__(self, frame_borrow):
        self.frame_borrow = frame_borrow
        self.label = Label(self.frame_borrow, text="대여/반납 정보창")
        self.label.pack()
...
class Window_Category:
    def __init__(self, frame_category):
        self.frame_category = frame_category
        self.label = Label(self.frame_category, text="카테고리")
        self.label.pack()


###### 다섯번째 버튼을 누르면 열리는 페이지
class Window_Publisher:
    def __init__(self, frame_publisher):
        self.frame_publisher = frame_publisher
        self.label = Label(self.frame_publisher, text="출판사")
        self.label.pack()